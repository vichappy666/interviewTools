...
- **📱 手机 / 局域网访问（默认开启）**：默认就绑 `0.0.0.0`，Mac 和手机 / iPad 连同一 WiFi，直接在手机浏览器打开 🌐 对话框里「手机 / 同 WiFi」那条 URL 即可，无需额外勾选（自动避开 ClashX/Surge 假 IP）
- **📌 跨 App 置顶（macOS，按需开启）**：点顶栏 📌 按钮开启 —— PyObjC 把 NSWindow level 抬到 `NSStatusWindowLevel`，并设置 `CanJoinAllSpaces + FullScreenAuxiliary`，切到别的 App / Space / 全屏应用都浮在最上层（类似微信视频小窗）。默认不置顶，避免开发/对比查资料时挡住其他窗口
...
8. 想在浏览器里用时，点 **🌐** 弹对话框显示本机 + 局域网 URL，可一键打开或复制
9. 想在切别的 App 时仍保持窗口可见，点 **📌** 开启置顶（默认**不**置顶）；再点一下取消
10. 想在手机 / iPad 上看：默认已开。手机连同一 WiFi → 点 **🌐** 弹对话框 → 复制「手机 / 同 WiFi」那条 URL（形如 `http://192.168.x.x:8765`）到手机浏览器打开
 ...
 (Qt)          ↓ 每句独立判断           Worker.ask(text)
              ...
              (可跨条划选)              key  script full
                    ...
                    └────── 同一批 Signal ────┤
                              ...
                              ├─ 状态快照 (供新连接补齐)
                              ├─ aiohttp HTTP/WS @ 0.0.0.0:8765 (本机 + 同 WiFi)
                              └─ 广播给所有浏览器 + 接受入站命令
                                             ▼
                                      浏览器镜像 UI
                              (HTML + vanilla JS，无构建)
...
- **跨 App 置顶（按需）**：macOS 下 `Qt.Tool` 对应 NSPanel，默认 App 失焦会 `orderOut:`；📌 按下时通过 PyObjC 对 NSWindow 调 `setLevel_(NSStatusWindowLevel)` + `setHidesOnDeactivate_(False)` + `setCanHide_(False)` 强制浮在所有 App 之上。默认不开启
...
- **局域网 IP 探测**：`get_lan_ip()` 三重兜底（UDP 路由表 / hostname 解析 / mDNS .local）+ 打分系统，`192.168 > 10 > 172.16-31 > 其他`，显式扣分 `198.18/15`（常被 ClashX/Surge 代理占用）和 `169.254` 链路本地
...
> - `enabled: false` 彻底关闭内置 Web 服务
> - `host: "0.0.0.0"`（默认）同时允许本机 + 同 WiFi 的手机 / iPad / 同事机器访问
> - `host: "127.0.0.1"` 锁成仅本机访问 —— 需要手动改 `~/.interview_assistant/config.json`，UI 没提供开关
> - ⚠️ 默认开启了局域网访问 = 同 WiFi 下任何人都能看你面试过程并替你发问，公共 WiFi 请关掉
...
A: 点顶栏 **📌** 按钮开启置顶（默认**不**置顶）。实现是 PyObjC 改 NSWindow level 到 `NSStatusWindowLevel`，同时 `setHidesOnDeactivate_(False)`，这样在 App 失焦、全屏应用、Space 切换下都能浮在最上层。仍有问题通常是 PyObjC 没装好，检查 `pip install pyobjc-framework-Cocoa` 是否成功。
...
**Q: 手机打开的地址和本机一样吗?**  
A: 不一样。本机是 `http://127.0.0.1:8765`（回环地址，只本机能连），手机要用 Mac 的局域网 IP，形如 `http://192.168.x.x:8765`。同一端口、同一个服务，只是换个地址访问。🌐 对话框两条都会列出来。

**Q: 手机怎么打开 Web UI?**  
A: (1) Mac 和手机连同一 WiFi；(2) 点顶栏 **🌐** 弹出对话框；(3) 复制「手机 / 同 WiFi」那条 URL 到手机浏览器。默认已支持，不用勾选或改配置。首次启动 macOS 防火墙会弹窗问是否允许 Python 入站，点允许。

**Q: 启动日志里打印的 LAN IP 是 `198.18.x.x` 或别的怪地址?**  
A: 那是 ClashX / Surge 等代理软件虚拟出来的 IP，不是真的 LAN IP。`get_lan_ip()` 已过滤这类地址，优先选 `192.168.x.x`、`10.x.x.x`、`172.16-31.x.x`。如果过滤后检测不到，暂时关掉代理再启动，或直接在终端跑 `ipconfig getifaddr en0` 拿真实 IP 手动拼 URL。
